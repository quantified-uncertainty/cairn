...
description: This model maps capability levels to risk activation thresholds. It
  identifies 15-25% benchmark performance as indicating early risk emergence,
  with 50% marking qualitative shift to complex autonomous execution.
sidebar:
  order: 16
quality: 4
...
ratings:
  ...
  completeness: 5
importance: 5
llmSummary: This model maps specific AI capabilities to risk activation
  thresholds across five dimensions (domain knowledge, reasoning depth, planning
  horizon, strategic modeling, autonomous execution). It identifies that most
  critical risks activate when systems cross 15-25% benchmark performance
  thresholds, with 50% representing qualitative shifts to complex autonomous
  execution, and predicts several threshold crossings in 2025-2027.
---
...
|-----------|---------|---------|---------|---------|
| **Domain Knowledge** | Undergraduate | Graduate | Expert | Superhuman |
| **Reasoning Depth** | Simple (2-3 steps) | Moderate (5-10) | Complex (20+) | Superhuman |
| **Planning Horizon** | Immediate | Short-term (hrs) | Medium (wks) | Long-term (months) |
| **Strategic Modeling** | None | Basic | Sophisticated | Superhuman |
| **Autonomous Execution** | None | Simple tasks | Complex tasks | Full autonomy |
...
The capability threshold model is uniquely important because it determines *when* other risks become salient. Understanding thresholds enables proactive rather than reactive risk management.

| Factor | Estimate | Confidence | Notes |
|--------|----------|------------|-------|
| Number of critical thresholds in next 5 years | 4-8 | Medium | Across misuse, control, structural risks |
| Average warning time before threshold crossing | 6-18 months | Medium | Varies by risk category |
| P(at least one threshold crossed unprepared) | 50-80% | Medium | Current monitoring insufficient |
| Expected cost of unprepared threshold crossing | $100B-$10T | Very Low | Wide range reflects scenario uncertainty |
| Value of 12-month additional warning time | $10B-$500B | Very Low | Depends on response effectiveness |

### Comparative Ranking

The capability threshold model is a **meta-model** that informs prioritization across other risks:

1. **Severity**: 7/10 - Model quality affects all downstream risk assessment
2. **Probability of model error**: 6/10 - Thresholds may be miscalibrated
3. **Neglectedness**: 4/10 - Significant attention but fragmented approaches
4. **Tractability**: 6/10 - Measurement infrastructure buildable

The strategic value of this model lies in:
- **Timing intervention**: Allocating resources before thresholds cross
- **Prioritization**: Focus on nearest thresholds first
- **Coordination**: Providing shared framework for labs and regulators

### Resource Implications

Current global investment in capability measurement/forecasting: ~$30-80M annually
Estimated adequate funding level: ~$100-250M annually (2-3x increase)

Priority investments:
1. **Standardized capability benchmarks** ($30-60M/year): Enable consistent threshold tracking
2. **Independent evaluation infrastructure** ($25-50M/year): Third-party verification
3. **Forecasting methodology** ($15-30M/year): Improve threshold prediction
4. **Early warning systems** ($20-40M/year): Operational monitoring and alerting

### Key Cruxes

| Crux | If True | If False | Current Assessment |
|------|---------|----------|-------------------|
| Thresholds are sharp rather than gradual | Clear triggers for action possible | Continuous risk scaling requires different approach | 55% sharp for most risks |
| Current benchmarks measure safety-relevant capabilities | Existing evals sufficient with expansion | Need fundamentally new evaluations | 40% sufficiently relevant |
| Capability growth continues exponentially | Thresholds may cross rapidly | More time to prepare | 65% continues exponentially |
| Thresholds can be measured before crossing | Proactive response possible | Reactive response only | 60% can be measured early |
