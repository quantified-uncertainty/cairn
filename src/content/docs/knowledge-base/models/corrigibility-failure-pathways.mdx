...
description: This model maps pathways from AI training to corrigibility failure.
  It estimates 60-90% failure probability for capable optimizers with unbounded
  goals, reducible by 40-70% through targeted interventions.
ratings:
  novelty: 4
  rigor: 4
  actionability: 5
...
quality: 4
importance: 85
llmSummary: This model systematically maps six pathways to AI corrigibility
  failure with quantified probability estimates (60-90% for capable optimizers)
  and intervention effectiveness (40-70% reduction through targeted approaches).
  It provides actionable analysis of failure mechanisms including instrumental
  convergence, goal preservation, and deceptive corrigibility with specific
...
Corrigibility failure represents one of the highest-magnitude AI safety risks, potentially affecting the long-term trajectory of human civilization. The expected impact can be estimated as follows:

| Factor | Estimate | Confidence | Notes |
|--------|----------|------------|-------|
| P(catastrophic outcome \| corrigibility failure at AGI) | 60-90% | Medium | Depends on capability level and failure mode |
| P(corrigibility failure \| advanced AI without intervention) | 85-98% | Medium-High | Based on pathway analysis above |
| P(corrigibility failure \| advanced AI with best current interventions) | 40-70% | Low | Interventions reduce but do not eliminate risk |
| Expected severity (1-10 scale) | 9.2 | Medium | Near-civilizational impact if combined with high capability |
| Potential affected population | 8-10 billion | High | Global scope for advanced AI scenarios |

### Comparative Ranking

Among AI safety concerns, corrigibility failure ranks as **Tier 1 (Critical Priority)** based on:

1. **Severity**: 9.5/10 - Potentially irreversible loss of human control
2. **Probability**: 7/10 - High baseline probability without intervention
3. **Neglectedness**: 5/10 - Some research attention but insufficient relative to risk
4. **Tractability**: 4/10 - Difficult but not impossible; key cruxes identified

Compared to other major AI risks:
- More fundamental than misuse risks (enables rather than constitutes harm)
- Higher probability than deceptive alignment but lower than reward hacking
- More tractable than value alignment but less tractable than AI control

### Resource Implications

Current global investment in corrigibility research: ~$20-50M annually
Estimated adequate funding level: ~$200-500M annually (4-10x increase needed)

Priority research investments:
1. **Formal corrigibility theory** (estimated need: $30-50M/year): Mathematical foundations for stable corrigibility
2. **Interpretability for corrigibility verification** ($50-100M/year): Methods to verify corrigibility claims
3. **AI control as corrigibility backup** ($40-80M/year): Containment assuming potential failure
4. **Training methods for corrigibility** ($30-60M/year): Techniques producing robust corrigibility

### Key Cruxes

The strategic importance of corrigibility research depends on several key empirical and theoretical uncertainties:

| Crux | If True | If False | Current Assessment |
|------|---------|----------|-------------------|
| Stable corrigibility is theoretically achievable | Research investment highly valuable | Pivot to AI control approaches | 55% likely achievable |
| Current training can produce stable corrigibility | Incremental improvements sufficient | Need fundamental breakthroughs | 30% likely sufficient |
| Interpretability can verify corrigibility | Verification possible before deployment | Must rely on behavioral testing | 45% likely possible |
| Corrigibility trades off with capability | Expect permanent safety tax | Win-win solutions possible | 60% likely some tradeoff |
