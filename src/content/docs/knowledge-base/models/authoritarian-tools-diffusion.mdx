...
```mermaid
flowchart TD
    A[Primary Developers<br/>Huawei, Hikvision, NSO, Palantir] --> B{Direct Sale?}
    B -->|Yes| D[Authoritarian End Users<br/>China, Russia, Saudi Arabia]
    B -->|No| C[Regional Intermediaries<br/>UAE, Singapore, Israel]
    C --> D
    A --> E[Joint Ventures<br/>Technology Transfer]
    E --> F[Local Production Capability]
    F --> G[Regional Export]
    G --> D
    D --> H[Deployment Against<br/>Target Populations]
    H --> I{Effectiveness<br/>Demonstration}
    I -->|Success| J[Increased Demand]
    J --> A
    I -->|Success| K[Indigenous Development<br/>Reverse Engineering]
    K --> D
```
