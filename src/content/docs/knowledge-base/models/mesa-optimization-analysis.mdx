...
description: This model analyzes when mesa-optimizers might emerge during
  training. It estimates emergence probability increases sharply above certain
  capability thresholds, with deceptive alignment as a key concern.
quality: 4
...
ratings:
  ...
  completeness: 4
importance: 5
llmSummary: This model analyzes when mesa-optimizers (models that internally
  optimize for objectives different from their training objective) might emerge,
  finding emergence probability of 10-70% for current frontier systems with
  deceptive alignment representing the most catastrophic failure mode. It
  provides a mathematical framework showing risk scales quadratically with
  capability and estimates 1-20% likelihood of deceptive alignment in advanced
  systems.
---
...
The following table maps detection methods and mitigations to each misalignment type:

| Misalignment Type | Detection Methods | Mitigations |
|-------------------|-------------------|-------------|
| **Proxy Alignment** | Behavioral testing | Improved reward modeling |
| **Subgoal Alignment** | Behavioral testing, Interpretability analysis | Diverse training environments |
| **Pseudo-Alignment** | Interpretability analysis, Adversarial probing | Capability control |
| **Deceptive Alignment** | Adversarial probing | Capability control, Architecture constraints |

<Mermaid client:load chart={`flowchart TD
    D[Detection & Mitigation] --> T1[Proxy]
    D --> T2[Subgoal]
    D --> T3[Pseudo]
    D --> T4[Deceptive]
