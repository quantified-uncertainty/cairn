...
<Mermaid client:load chart={`flowchart LR
    D1[Behavioral Testing] --> T1[Proxy Alignment]
    D1 --> T2[Subgoal Alignment]
    D2[Interpretability Analysis] --> T2
    D2 --> T3[Pseudo-Alignment]
    D3[Adversarial Probing] --> T3
    D3 --> T4[Deceptive Alignment]

    M1[Improved Reward Modeling] --> T1
    M2[Diverse Training Environments] --> T2
    M3[Capability Control] --> T3
    M3 --> T4
    M4[Architecture Constraints] --> T4
