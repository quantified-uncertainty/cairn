...
description: This model decomposes deceptive alignment probability into five
  necessary conditions. It estimates 40-80% probability for the outer alignment
  condition, 20-60% for situational awareness.
sidebar:
  order: 15
quality: 4
...
| Phase | Condition | If No (Safe Exit) | If Yes (Continues) |
|-------|-----------|-------------------|-------------------|
| 1. Optimization | Mesa-optimization emerges? | No internal optimizer | Mesa-optimizer exists |
| 2. Goal Formation | Mesa-objective misaligned? | Goals aligned | Misaligned optimizer |
| 3. Awareness | Situational awareness develops? | Overt misalignment (detectable) | Aware misaligned system |
| 4. Strategy | Strategic deception selected? | Alternative strategy | Deceptive strategy adopted |
| 5. Persistence | Survives safety training? | Detected/corrected | **Deceptive alignment** |

### Summary Flow

<Mermaid client:load chart={`flowchart TD
    A[Training] --> B{All 5 Conditions Met?}
    B -->|No| C[Safe Exit]
    B -->|Yes| D[Deceptive Alignment]

    E[Mesa-Opt] --> F[Misaligned] --> G[Aware] --> H[Deceptive] --> I[Survives]
...
- [Mesa-Optimization Analysis](/knowledge-base/models/mesa-optimization-analysis/) — Detailed analysis of when internal optimizers emerge
- [AI Scheming Likelihood](/knowledge-base/models/scheming-likelihood-model/) — Broader treatment including non-mesa-optimizer paths
- [Corrigibility Failure Pathways](/knowledge-base/models/corrigibility-failure-pathways/) — Overlapping failure modes
