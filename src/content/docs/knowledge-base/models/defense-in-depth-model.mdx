...
description: This model analyzes how layered safety measures combine. Individual
  layers provide 20-60% coverage; independence between layers is critical for
...
importance: 85
llmSummary: This model provides a mathematical framework for analyzing layered
  AI safety measures, showing that independent layers with 20-60% individual
  failure rates can achieve combined failure rates as low as 1-3%, but
  correlated failures (especially from deception) can increase this to 12%+. The
  analysis includes specific quantitative assessments of five defense layers and
  identifies key failure correlation patterns.
...
<Mermaid client:load chart={`
flowchart TD
    A[Deceptive Model] --> B[Defeats Layers 1-3]
    B --> C[Training: Appears Aligned]
    B --> D[Evaluation: Passes Tests]
    B --> E[Runtime: Behaves Well]
    C --> F[Only Layers 4-5 Remain]
    D --> F
    E --> F
...
Defense in depth is the primary operational framework for achieving AI safety despite uncertain individual interventions. Its strategic importance lies in providing robust protection when no single defense is reliable enough.

| Factor | Estimate | Confidence | Notes |
|--------|----------|------------|-------|
| P(single-layer defense sufficient) | 10-40% | Low | No individual intervention is reliable |
| P(5-layer defense sufficient under independence) | 85-98% | Medium | If layers truly independent |
| P(5-layer defense sufficient with typical correlation) | 60-85% | Medium | Realistic scenario |
| Failure rate reduction from adding layers | 2-5x per layer | Medium | Depends on correlation |
| Cost multiplier for layered vs. single defense | 1.5-3x | Medium | Diminishing returns apply |

### Layer Effectiveness Estimates

The following table provides quantitative estimates for individual layer performance based on current implementations:

| Layer | Typical Failure Rate | Best-Case Failure Rate | Worst-Case Failure Rate | Correlation with Adjacent Layers |
|-------|---------------------|------------------------|-------------------------|----------------------------------|
| Training | 25-40% | 15% (advanced alignment) | 60% (misalignment) | 0.3-0.5 with Evaluation |
| Evaluation | 25-35% | 10% (comprehensive testing) | 50% (capability jump) | 0.3-0.5 with Runtime |
| Runtime | 30-50% | 15% (AI control) | 70% (capability escape) | 0.4-0.6 with Training |
| Institutional | 40-60% | 25% (strong governance) | 80% (regulatory capture) | 0.1-0.3 with Technical |
| Recovery | 20-40% | 10% (prepared response) | 60% (irreversible harm) | 0.1-0.2 with all layers |

### Comparative Ranking

Defense in depth ranks as **Tier 1 (Critical Priority)** for operational AI safety:

1. **Necessity**: 9/10 - Required given uncertainty in individual interventions
2. **Neglectedness**: 5/10 - Conceptually recognized but correlation analysis lacking
3. **Tractability**: 7/10 - Implementation straightforward; correlation reduction difficult
4. **Leverage**: 8/10 - Proper implementation dramatically reduces failure probability

### Resource Implications

Current global investment in defense-in-depth implementation: ~$100-300M annually (implicit in safety budgets)
Estimated adequate funding level: ~$300-800M annually (2-3x increase, focused on reducing correlation)

Priority investments:
1. **Interpretability for deception detection** ($50-120M/year): Reduces correlation in worst failure mode
2. **Independent evaluation infrastructure** ($40-90M/year): Reduces correlation between training and evaluation
3. **Institutional safety mechanisms** ($30-70M/year): Layer with lowest correlation to technical failures
4. **Recovery capability development** ($25-60M/year): Backstop when prevention fails

### Key Cruxes

| Crux | If True | If False | Current Assessment |
|------|---------|----------|-------------------|
| Layers can be made sufficiently independent | Defense in depth highly effective | Correlated failures dominate | 55% can achieve meaningful independence |
| Deception correlation can be broken | Technical layers remain valuable | Must rely on institutional/recovery | 40% can be broken |
| Institutional layers truly independent | Reliable backstop exists | All layers may fail together | 70% largely independent |
| Defense in depth works against advanced AI | Strategy remains viable | Need fundamentally different approach | 45% works at advanced levels |
