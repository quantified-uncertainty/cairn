...
description: This model analyzes how risk depends on which actors develop TAI.
  It maps actor incentive structures and identifies conditions where actor
  identity dominates technical factors.
...
ratings:
  ...
  completeness: 4
importance: 84.5
llmSummary: This model analyzes AI risk through the lens of which actors develop
  transformative AI, mapping current capabilities (US labs at 0.85, China at
  0.65, open-source at 0.55 vs frontier) and their incentive structures. It
  identifies that actor identity and competition dynamics may dominate technical
  safety factors in determining outcomes, with quantified estimates for various
  risk scenarios including unaligned singleton (0.08 expected loss) and
  multi-agent conflict (0.06 expected loss).
...
This model analyzes how risk depends on which actors develop TAI. It maps actor incentive structures and identifies conditions where actor identity dominates technical factors.
...
**Key Finding:** Open-source lag is shrinking from ~18 months to ~12 months. Malicious actor access lag remains 2-3 years but is highly uncertain.
