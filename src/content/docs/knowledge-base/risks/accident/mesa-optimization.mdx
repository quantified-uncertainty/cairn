...
maturity: Growing
quality: 2
llmSummary: Mesa-optimization occurs when trained AI systems develop their own
  internal optimization processes with objectives that may differ from the
  training objective. The page analyzes the likelihood (10-70% depending on
  perspective) and severity (high to catastrophic) of this inner alignment
  problem.
