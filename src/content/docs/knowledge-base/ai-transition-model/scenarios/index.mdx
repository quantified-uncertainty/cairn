...
Structural Scenarios are the intermediate pathways that connect [root factors](/knowledge-base/ai-transition-model/factors/) to [ultimate outcomes](/knowledge-base/ai-transition-model/outcomes/). They describe *how* parameter changes lead to catastrophe (or success)—the specific mechanisms and pathways that determine what kind of future we get.

The [AI Transition Model](/knowledge-base/ai-transition-model/) uses **three main structural scenarios**:
...
Each structural scenario has sub-variants that describe more specific pathways (e.g., "rapid" vs "gradual" AI takeover, "state" vs "rogue actor" catastrophe).
...
flowchart TD
    ...
    subgraph Scenarios["Structural Scenarios"]
    ...
    %% Root Factors to Structural Scenarios
    ...
    %% Structural Scenarios to Ultimate Outcomes
...
- **Pink**: Negative structural scenarios (catastrophes)
- **Orange**: Symmetric structural scenario (could entrench good or bad values)

---

## Structural Scenarios Summary

| Structural Scenario | Description | Sub-variants | Key Root Factors | Ultimate Outcomes |
|-------------------|-------------|--------------|------------------|-------------------|
...
## How Structural Scenarios Differ from Other Concepts
...
| **Structural Scenarios** | Intermediate pathways connecting factors to outcomes | "AI Takeover" |
| **Ultimate Outcomes** | High-level goals we care about | "Existential Catastrophe", "Long-term Trajectory" |

**Key distinction**: A *risk* like "deceptive alignment" is a mechanism that could happen. A *structural scenario* like "AI Takeover" is the outcome that results if such mechanisms play out. Multiple risks can contribute to a single structural scenario.
...
Without this layer, the connection between "Misalignment Potential increasing" and "Existential Catastrophe increasing" is abstract. Structural scenarios show the specific pathway: alignment fails → AI develops misaligned goals → AI takes over → catastrophe.

### 2. Enables Different Intervention Strategies

Different structural scenarios require different interventions:
...
Structural scenarios map directly onto scenarios that organizations can plan for. Rather than asking "what if Existential Catastrophe increases?", planners can ask "what if we're heading toward a Human-Caused Catastrophe?"

### 4. Connects to Existing Threat Models

Each structural scenario corresponds to threat models discussed in the AI safety literature:
- Carlsmith's six-premise argument → AI Takeover scenarios
- Christiano's "What Failure Looks Like" → Gradual AI Takeover
- Ord's "The Precipice" risk categories → Multiple structural scenarios
...
### For Analysts
- Map specific risks to the structural scenarios they could produce
- Estimate which structural scenarios are most likely given current parameter trends
- Identify which parameters to prioritize based on which structural scenarios concern you most

### For Policymakers
- Design interventions targeted at preventing specific structural scenarios
- Coordinate across domains (a single structural scenario may require multiple types of intervention)
- Track early warning signs for each structural scenario

### For Researchers
- Use structural scenarios to frame research priorities
...
- [Root Factors](/knowledge-base/ai-transition-model/factors/) — The parameter groupings that feed into structural scenarios
- [Ultimate Outcomes](/knowledge-base/ai-transition-model/outcomes/) — The high-level goals structural scenarios affect
