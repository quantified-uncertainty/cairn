...
<a href="/parameter-graph/" class="sl-link-card" style="display: block; padding: 1.5rem; border: 2px solid var(--sl-color-accent); border-radius: 0.5rem; text-decoration: none; margin: 1rem 0 2rem 0; background: var(--sl-color-accent-low);">
  <strong style="font-size: 1.25rem; display: block; margin-bottom: 0.5rem;">🔗 View Full Interactive Model →</strong>
  <span style="color: var(--sl-color-text);">Explore the parameter graph with hover highlighting, click-to-inspect nodes, and fullscreen mode. This is the canonical visualization of the three-layer causal structure.</span>
</a>
...
```
Root Factors (5) → Structural Scenarios (3) → Ultimate Outcomes (2)
```
