...
sidebar:
  order: 11
quality: 3
llmSummary: "ARC is an influential AI safety organization founded by Paul
  Christiano in 2021, operating two divisions: ARC Theory (focused on
  fundamental alignment problems like Eliciting Latent Knowledge) and ARC Evals
  (conducting capability evaluations of frontier models for dangerous
  behaviors). The organization has significantly shaped the field by
  establishing evaluations as a key governance tool and developing worst-case
  alignment approaches that assume AI systems might be adversarial."
