...
description: >-
  This framework analyzes compute monitoring approaches for AI governance,
  finding that cloud KYC (targeting 10^26 FLOP threshold) is implementable now
  via the three major providers controlling 60%+ of cloud infrastructure, while
  hardware-level governance faces 3-5 year development timelines. The EU AI Act
  uses a lower 10^25 FLOP threshold. Evasion through on-premise compute and
  jurisdictional arbitrage remains the primary limitation.
sidebar:
  order: 3
quality: 80
llmSummary: >-
  Compute monitoring establishes visibility into AI training runs through cloud
  KYC requirements (implementable now via customer verification, reporting
  obligations, and access controls at major providers) and hardware-level
  governance (longer-term via chip registration and on-chip monitoring). Cloud
  KYC targets the ~10^26 FLOP threshold from EO 14110, but faces evasion through
  on-premise compute, while hardware monitoring faces substantial technical
  challenges around performance overhead, security, and backward compatibility.
