...
sidebar:
  order: 2
quality: 3
llmSummary: Analyzes compute thresholds as regulatory triggers, documenting
  current implementations (EU AI Act at 10^25 FLOP, US EO at 10^26 FLOP) and
  identifying algorithmic efficiency improvements (~2x/year) as the core
  challenge that may make thresholds obsolete within 3-5 years. Provides
  concrete assessment that thresholds are useful short-term proxies but may need
  evolution toward capability-based triggers.
