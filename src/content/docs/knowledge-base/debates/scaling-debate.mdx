...
description: The debate over whether scaling compute and data is sufficient for
  AGI or if we need new paradigms
sidebar:
  order: 2
importance: 5
quality: 4
llmSummary: This page examines the central debate over whether scaling current
  AI approaches (transformers, neural networks) is sufficient to reach AGI, or
  if fundamental new paradigms are needed. It maps arguments from scaling
  optimists citing predictable performance improvements versus skeptics pointing
  to reasoning failures and architectural limitations.
---
