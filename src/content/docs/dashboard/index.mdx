...
|------|:-------:|:----------:|:---:|----------|
{enhancementQueue.map(p => `| [${p.title}](${p.path}) | ${p.quality} | ${p.importance} | ${p.gap} | ${p.category} |`).join('\n')}

To enhance these pages, run:
```bash
node scripts/document-enhancer.mjs enhance --min-imp 30 --max-qual 3 --limit 10
```
