...
interface EntityWithGraph {
  ...
  causeEffectGraph?: {
    ...
    nodes: Array<{
      ...
      color?: string;  // Tailwind color name (e.g., "rose", "violet", "teal")
    }>;
    edges: Array<{
      id?: string;
      source: string;
...
export default function DiagramViewer({ entityId: propEntityId }: DiagramViewerProps) {
  ...
  // Default view mode is determined by whether diagram has semantic colors
  ...
  // Set default view mode based on whether diagram has semantic colors
  useEffect(() => {
    if (hasInitializedViewMode || !entityId) return;
    ...
    // Check if any node has a semantic color
    const hasSemanticColors = graph.nodes.some(node => node.color);

    // If diagram has colors, use "Default view" (undefined); otherwise use sensitivity
    setScoreHighlight(hasSemanticColors ? undefined : 'sensitivity');
    setHasInitializedViewMode(true);
  }, [entityId, hasInitializedViewMode]);
