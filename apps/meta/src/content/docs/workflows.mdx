...
<Aside type="tip" title="Using These Workflows">
These workflows are designed for Claude Code (CLI). Copy the prompts directly and adapt the `[bracketed]` sections for your specific task.
</Aside>
...
| Workflow | Input | Output | Time | Cost |
|----------|-------|--------|------|------|
| **Page Improvement** | Low-quality page | Q5 page with tables, diagrams, citations | 15-30 min | \$3-5 (Opus) |
| **Research Report** | Topic question | Comprehensive report with causal factors | 30-60 min | \$5-10 (Opus) |
| **Causal Diagram** | Entity + research | YAML causeEffectGraph | 10-20 min | \$2-4 (Opus) |
| **Quantitative Estimates** | Factor list | Documented estimates with reasoning | 20-40 min | \$3-6 (Opus) |
| **Full Topic Pipeline** | Topic | Report + diagram + estimates | 1-2 hours | \$10-20 (Opus) |
...
- Page has `quality < 4` and `importance > 50`
- Page is missing tables, diagrams, or citations
- Content is bullet-heavy or sparse
...
- Need deep understanding of a topic before creating diagrams
- Filling gaps in existing knowledge base coverage
...
The `/research-report` skill provides detailed guidance. You can also invoke it directly:
...
Reports go to: `src/content/docs/internal/research-reports/{topic-id}.mdx`

### Post-Workflow

After completing a research report, the Causal Factors section can be directly used to create a cause-effect diagram.
...
- Entity in `ai-transition-model.yaml` lacks a `causeEffectGraph`
- Research report has been completed with causal factors
- Modeling a new factor or relationship
...
Diagrams are added to entities in: `src/data/entities/ai-transition-model.yaml`

Find the entity by searching for `id: tmc-{factor-name}`.

### View Result

- In development: `http://localhost:4321/diagrams?entity={entity-id}`
...
- Populating the estimates table at `/ai-transition-model/table/`
- Adding quantitative backing to qualitative claims
- Comparing different expert positions
...
### Integration with Table Page

Estimates can be added to entity frontmatter or to dedicated estimate collections. The table page at `/ai-transition-model/table/` aggregates these.

### Example Factors

Common factors needing estimates:
- Timeline estimates (when will X capability emerge?)
- Probability estimates (likelihood of alignment success)
- Resource estimates (compute, talent, funding)
- Impact magnitudes (severity of different risks)
...
**The Problem:** LLM outputs vary based on model, prompt phrasing, and simulated perspective. A single estimate may not represent the true range of reasonable answers.
...
| **Personas** | Simulated expert perspectives | Skeptic, optimist, domain expert, policy analyst |
...
**When to Use:**
- High-stakes estimates that inform decisions
- Contentious topics where reasonable people disagree
...
- Adding a new factor to the AI Transition Model
- Creating comprehensive coverage from scratch
...
This is the first step in building full coverage. Focus on:
...
This report will inform diagram creation and estimates.
```
...
### Step 4: Create/Update Knowledge Base Page
...
Use the enhancement queue to track pipeline progress:
...
When working on multiple pages/topics, you can parallelize:
...
- [Automation Tools](/automation-tools/) - CLI tools reference
- [Cause-Effect Diagram Style Guide](/cause-effect-diagrams/) - Diagram creation details
- [Research Report Style Guide](/research-reports/) - Report format details
- [Knowledge Base Style Guide](/knowledge-base/) - Page format details
