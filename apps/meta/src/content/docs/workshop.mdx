...
<Aside type="tip" title="Want to Try This Right Now?">
You don't need Claude Code installed to get started. Open any LLM — [Claude.ai](https://claude.ai), ChatGPT, Gemini, or whichever you prefer — and start working through the process:

1. **Pick a topic** and ask the LLM to help you outline a structure (sections, pages, key questions)
2. **Draft content** by pasting the prompts from the [workshop walkthrough below](#workshop-walkthrough) — the LLM can generate page outlines, comparison tables, and initial drafts
3. **Plan your wiki** — ask it to suggest what pages to create, how to organize sections, and what to cover

This gets you 80% of the thinking done. When you're ready to turn it into an actual website, come back here for the Claude Code workflow — or copy-paste the LLM's output into any static site framework manually.
</Aside>
...
1. **Node.js 18+** — [nodejs.org](https://nodejs.org/) (LTS version). Run `node --version` to check — if it says `v16` or lower, you need to update.
2. **Claude Code** — `npm install -g @anthropic-ai/claude-code`, then run `claude` once to authenticate ([cost info below](#installing-claude-code)). If you get `EACCES` permission errors on macOS/Linux, try `sudo npm install -g @anthropic-ai/claude-code`.
...
**Corporate or university laptop?** If admin restrictions prevent installing Node.js or npm packages, use the web app instead.

**Prefer to skip local setup?** The [Claude Code Web App](/claude-code-web-app/) lets you do everything in the browser — no terminal or Node.js required.
</Aside>
...
Claude Code is available as a **CLI** (terminal), a **VS Code and JetBrains extension**, and via the **[claude.ai/code](https://claude.ai/code) web interface**. This workshop walkthrough assumes the local CLI, but see the **[Claude Code Web App guide](/claude-code-web-app/)** if you'd rather work entirely in the browser.
...
**Rate limits:** Pro subscription users may hit rate limits if making very rapid requests — if you see "too many requests" errors, wait 30-60 seconds and try again. Max plan (\$100/mo or \$200/mo) has more generous limits. Not sure which billing you're on? Check [claude.ai/settings](https://claude.ai/settings) for your plan, or [console.anthropic.com](https://console.anthropic.com/) for API billing.
</Aside>
...
6. **Check the browser and start expanding**
   ...
   **Iterate in layers — this is where quality comes from.** Your first prompt produces a scaffold; the next 2-3 prompts are what make it actually good. Here's what a real iteration sequence looks like on a single page:
   ...
   <Aside type="caution" title="Watch for generic output">
   After the initial scaffold, all your pages will likely sound the same — generic "this is an important topic" prose. This is normal. The quality comes from iteration. If everything sounds identical, give Claude a specific voice: "Write in the style of a GiveWell cost-effectiveness analysis" or "Match the tone of 80,000 Hours career profiles." Also watch for invented citations — Claude will confidently link to papers and URLs that don't exist. Ask it to web-search before writing factual sections.
   </Aside>
...
<Aside type="tip" title="Using the Claude Code Web App?">
See the **[Web App guide's deployment section](/claude-code-web-app/#deploying-your-site)** for step-by-step GitHub Pages and Vercel setup tailored to the web app workflow.
</Aside>
...
- Run `node --version` — you need 18+. If it says `v16` or lower, or "command not found", install from [nodejs.org](https://nodejs.org/) (LTS version)
...
- Check the `base` setting in `astro.config.mjs`. For GitHub Pages with a repo subpath, you need `base: '/repo-name/'`. For Vercel/Netlify with a custom domain, `base` should be `'/'` or omitted.
