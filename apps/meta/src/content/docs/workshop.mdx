...
4. **A way to preview your site** — the web app can't preview locally, so you'll deploy to see your site. **GitHub Pages** works with no extra account (just ask Claude to set it up), or use [Vercel](https://vercel.com/)/[Netlify](https://www.netlify.com/) for faster deploys. Details in [Setting Up Claude Code](#setting-up-claude-code).
...
<Tabs>
  <TabItem label="Web App (easiest)">
    ...
    Since the web app can't show you a live preview, **set up auto-deployment right after creating your project.** This gives you a live URL that rebuilds every time Claude commits. Pick one:

    **GitHub Pages (simplest — no extra account needed):**
    Once your project is scaffolded, ask Claude in the web app:
    > "Set up GitHub Pages deployment with a GitHub Actions workflow. Configure the Astro site for GitHub Pages output."

    Claude will create the workflow file and update your Astro config. After it commits and pushes, go to your repo's **Settings → Pages** and select **GitHub Actions** as the source. Your site will be at `https://your-username.github.io/your-repo/`.

    **Vercel (faster deploys, slightly nicer URLs):**
    ...
    Either way, keep your deployed URL open in another browser tab — it becomes your preview. Deploys typically take 30-90 seconds.
    </Aside>
  </TabItem>
...
<Tabs>
  <TabItem label="Web App">
    ...
    - **View your deployed site** in a separate browser tab (via Vercel/Netlify — see [Setting Up Claude Code](#setting-up-claude-code) for setup)

    No split-pane terminal setup needed. The main difference from local setup: instead of watching a localhost dev server, you'll check your deployed URL after Claude commits.
  </TabItem>
...
3. **Start the dev server and Claude Code**
   ...
   <Tabs>
     <TabItem label="Web App">
       You don't need a local dev server — you're already chatting with Claude, and your deployed URL (GitHub Pages, Vercel, or Netlify) will show you the site after each commit. Ask Claude to commit and push after major changes so you can check the result.
   ...
   **Checkpoint:** You should see a working docs site (in your browser or the web app preview pane).
...
<Tabs>
  <TabItem label="GitHub Pages (free)">
    Ask Claude: "Create a GitHub Actions workflow that builds my Astro site and deploys to GitHub Pages."

    Then go to your repo's **Settings → Pages** and select **GitHub Actions** as the source. Your site will be at `https://your-username.github.io/your-repo/`.

    This is the simplest option if your code is already on GitHub (which it is automatically if you used the web app).
  </TabItem>
