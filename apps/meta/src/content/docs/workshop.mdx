...
- **Warning:** The elevator is out of order. Plan to walk up three flights of stairs.
...
*A shorter version of this workshop will also be given at EAG SF in February 2026.*
...
[Claude Code](https://docs.anthropic.com/en/docs/claude-code/overview) is a command-line tool that lets you use Claude AI directly in your terminal. It can read, write, and edit files in your project, run commands, and search the web — all through a conversational interface. You type instructions, Claude executes them on your codebase.
...
|---------|-------------|-------|
| [CAIRN](https://ea-crux-project.vercel.app/) | Comprehensive AI safety knowledge navigator — risks, interventions, organizations, key uncertainties | ~500 pages, YAML data layer, custom components |
...
The source for both is at [github.com/quantified-uncertainty/cairn](https://github.com/quantified-uncertainty/cairn). CAIRN took months of iterative work, but the basic structure was scaffolded in a single session.
...
**If using the API instead:** You can sign up for an Anthropic API account at [console.anthropic.com](https://console.anthropic.com/). Before the workshop, **set a spending limit** — go to Settings → Limits and set a hard limit (e.g., \$10). Without a limit, a busy workshop session could easily run up \$20-30+. Start with a small limit and increase it if needed.
</Aside>
...
2. **Scaffold the project**
   ...
   The wizard will ask a few questions — here's what to pick:
   - **Where should we create your new project?** → `./my-wiki` (default is fine)
   - **Install dependencies?** → Yes
   - **Initialize a new git repository?** → Yes
   - **TypeScript?** → Strictest (or whatever you're comfortable with)
...
3. **Start the dev server** (in Window 2)
   ...
   npm run dev
   ```
   Open `http://localhost:4321` in your browser. You should see the default Starlight template with a "Welcome to Starlight" page.
...
5. **Create your entire page structure in one prompt**
   ...
   <Tabs>
     ...
     <TabItem label="Template: Research Wiki">
       ...
       Also:
       - Remove the default Starlight example pages
       - Update astro.config.mjs to use autogenerate for
...
The fastest way — ask Claude to create one:
...
For [CAIRN](https://ea-crux-project.vercel.app/) (~500 pages), we eventually added:
...
You don't need any of this to start — but it shows where the approach can go. The source is open at [github.com/quantified-uncertainty/cairn](https://github.com/quantified-uncertainty/cairn).
...
| **CAIRN source** | [github.com/quantified-uncertainty/cairn](https://github.com/quantified-uncertainty/cairn) |
