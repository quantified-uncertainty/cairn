...
Set up **one** of these options **before** arriving so we can dive straight into building:

**Option A — Web App (easiest, no installation):**
1. **Claude Pro or Max subscription** — sign up at [claude.ai](https://claude.ai/) if you haven't
2. **Go to [claude.ai/code](https://claude.ai/code)** — this gives you a development environment in your browser (terminal, file editor, and Claude — no local setup needed)
3. **A GitHub account** — the web app creates a repo for your project automatically
4. **A free [Vercel](https://vercel.com/) account** (sign up with GitHub) — the web app can't preview your site locally, so you'll connect your GitHub repo to Vercel to get a live URL that auto-updates on every push. Takes ~2 minutes, details in [Setting Up Claude Code](#setting-up-claude-code).
...
If you're unsure, **go with Option A** — it works entirely in the browser and you can always switch to a local setup later. Basic familiarity with Claude Code and the command line will be useful for either option.
</Aside>
...
Claude Code is available as a **CLI** (terminal), a **VS Code and JetBrains extension**, and via the **[claude.ai/code](https://claude.ai/code) web interface**. The web interface is the easiest way to get started — no local installation needed. The same workflow applies everywhere.
...
<Tabs>
  <TabItem label="Web App (easiest)">
    No installation needed — Claude Code runs entirely in your browser.

    1. Go to **[claude.ai/code](https://claude.ai/code)**
    2. Sign in with your Claude account (requires Pro or Max subscription)
    3. Click **"Create project"** — this sets up a GitHub repo and cloud development environment automatically
    4. You get a built-in terminal and file editor — Claude reads and writes your project files directly

    The web app handles Node.js, npm, and file editing for you. However, **it can't preview your site in the browser** — you'll need to deploy to see what your site looks like (see the next section).

    <Aside type="tip" title="GitHub account needed">
    The web app stores your project in a GitHub repo. If you don't have a GitHub account, create one at [github.com](https://github.com/) — it's free.
    </Aside>

    <Aside type="note" title="Deploy early to preview your site">
    Since the web app can't show you a live preview, **connect your repo to Vercel right after creating your project.** This gives you a live URL that rebuilds every time you push.

    1. Go to [vercel.com](https://vercel.com/) and sign in with your GitHub account
    2. Click **"Add New Project"** → select the repo the web app created
    3. Vercel auto-detects Astro — just click **Deploy**
    4. You'll get a URL like `your-project.vercel.app`

    Keep this URL open in another browser tab — it becomes your preview. Deploys typically take 30-60 seconds. Now every time you ask Claude to commit and push, Vercel rebuilds automatically.

    **Why Vercel over GitHub Pages?** GitHub Pages requires manual config changes (`site` and `base` in astro.config), a GitHub Actions workflow, changing a repository setting in the GitHub UI, and has [known issues with Starlight base paths](https://github.com/withastro/starlight/discussions/2158) that break links. Vercel just works with zero configuration.
    </Aside>
  </TabItem>
  <TabItem label="CLI (local)">
    Install Claude Code locally for full control over your environment.
  ...
  <TabItem label="VS Code Extension">
    If you use VS Code, the Claude Code extension gives you Claude in the sidebar alongside your editor.

    1. Search **"Claude Code"** in the VS Code Extensions marketplace
    2. Install and authenticate with your Anthropic account
    3. Use Claude directly in the VS Code sidebar while editing files and running the terminal below
  </TabItem>
</Tabs>

<Aside type="caution" title="Cost — Read Before the Workshop">
**Recommended: Use a Claude subscription.** Claude **Pro** (\$20/mo) or **Max** (\$100/mo or \$200/mo) subscriptions include Claude Code usage — no surprise bills. This is the easiest option for the workshop, and is **required** for the web app.

**If using the API with the CLI instead:** You can sign up for an Anthropic API account at [console.anthropic.com](https://console.anthropic.com/). Before the workshop, **set a spending limit** — go to Settings → Limits and set a hard limit (e.g., \$20). A typical 90-minute workshop session involves 15-30 substantial Claude interactions and costs roughly **\$5-15 on the API**. Pro subscription users should be fine but may hit rate limits if making very rapid requests.
</Aside>
...
<Tabs>
  <TabItem label="Web App">
    The web app at [claude.ai/code](https://claude.ai/code) gives you Claude Code and a terminal in one interface. You can:

    - **Chat with Claude** in the main panel
    - **See file changes** as Claude makes them
    - **Use the terminal** for git commands or manual operations
    - **View your deployed site** in a separate browser tab (via Vercel — see [Setting Up Claude Code](#setting-up-claude-code))

    No split-pane terminal setup needed. The main difference from local setup: instead of watching a localhost dev server, you check your Vercel URL after Claude commits and pushes.
  </TabItem>
...
|------|---------------|------------|----------|
| **[Obsidian](https://obsidian.md/)** + [Quartz](https://quartz.jzhao.xyz/) | AI plugins, local-first | Quartz generates a static site from your vault | Personal research wikis, interlinked notes |
| **[Notion](https://notion.so/)** | Built-in Notion AI | Notion Sites or share publicly | Team wikis, collaborative docs |
| **[Google NotebookLM](https://notebooklm.google.com/)** | Built-in (source-grounded) | Export only | Synthesizing a set of source documents, Q&A over your research |
...
2. **Scaffold the project**
   ...
   <Tabs>
     <TabItem label="Web App">
       In the Claude Code web app, just ask Claude to set up the project for you. In the chat, type:
       ...
       Or for a vanilla Starlight project:

       ```
       Create a new Astro Starlight project here.
       Run: npm create astro@latest -- --template starlight .
       Then run npm run dev.
       ```

       Claude will run the commands and install dependencies. After scaffolding, ask Claude to commit and push — this triggers your deployment so you can see the site.
     </TabItem>
     <TabItem label="Vanilla Starlight (local)">
     ...
     <TabItem label="QURI Starter (local)">
   ...
   **Checkpoint:** You should have a project with `src/content/docs/`, `astro.config.mjs`, and `package.json`.

3. **Start the dev server and Claude Code**

   <Tabs>
     <TabItem label="Web App">
       You don't need a local dev server — you're already chatting with Claude, and your Vercel URL will show you the site after each push. Ask Claude to commit and push after major changes so you can check the result.
   ...
   **Checkpoint:** You should see a working docs site (in your browser, or on your Vercel URL if using the web app).
...
9. **Save your progress**
   ...
   <Tabs>
     <TabItem label="Web App">
       Ask Claude: "Commit all changes with the message 'workshop: initial research wiki'" — or use the terminal in the web app to run git commands directly. Your project is already backed by a GitHub repo, so your work is saved.
     </TabItem>
     <TabItem label="Local CLI">
       In Window 3:
...
When you're ready to put your site online (or if you're using the web app, you may already have this set up):

<Tabs>
  <TabItem label="Vercel (recommended)">
    1. Push to GitHub (if not already — web app users are already there)
    2. Go to [vercel.com](https://vercel.com/), import your repo
    3. It auto-detects Astro — just click Deploy
    4. You'll get a URL like `your-project.vercel.app`

    Zero configuration needed. Auto-deploys on every push. This is what we recommend for the workshop.
  </TabItem>
  ...
  <TabItem label="GitHub Pages (free, more setup)">
    GitHub Pages is free but requires several manual steps:

    1. Ask Claude: "Create a GitHub Actions workflow that builds my Astro site and deploys to GitHub Pages. Set the `site` and `base` config in astro.config.mjs for my repo."
    2. **Manually** go to your repo's **Settings → Pages** and change the source to **GitHub Actions** (this step can't be automated)
    3. Your repo must be **public** on the free GitHub plan
    4. Your site will be at `https://your-username.github.io/your-repo/`

    <Aside type="caution" title="Base path gotchas">
    When deploying to `username.github.io/repo-name`, Astro requires setting `base: '/repo-name'` in the config. This means manually authored links in your Markdown (like `[about](/about)`) will break — they need to be relative links (`./about`) or include the base path. Starlight has [known issues](https://github.com/withastro/starlight/discussions/2158) with base paths affecting sidebar and hero links. If you want a hassle-free experience, use Vercel or Netlify instead.
    </Aside>
  </TabItem>
...
For [LongTermWiki](https://longtermwiki.com/) (~640 pages), we eventually added:
