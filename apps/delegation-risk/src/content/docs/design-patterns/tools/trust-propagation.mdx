  ...
  <div class="prop-container">
    ...
    <div class="prop-section">
      ...
      <select class="rule-select" id="prop-rule">
        ...
        <option value="weakest">At Least One Succeeds (parallel redundancy)</option>
      </select>
  ...
  <script>{`
    (function() {
      ...
      const ruleDescriptions = {
        ...
        'weakest': 'Effective trust = 1 - [(1-T₁)(1-T₂)...(1-Tₙ)] — at-least-one-succeeds (parallel redundancy): chain succeeds if any link succeeds'
      };
...
:::note[Which rule should I use?]
This is no longer an open choice. The framework's canonical rule is **multiplicative composition with a common-cause correction**: $(1-\rho)\prod T_i + \rho \min T_i$. The minimum rule is the fully-correlated limit ($\rho = 1$), not an alternative; the harmonic rule is retained in this tool only for comparison and is **incoherent for serial chains** (it can rate a chain above its weakest link). See [Risk Propagation: One Rule, Not a Menu](/delegation-risk/risk-propagation/) for the derivation and Monte Carlo evidence.
:::
...
**Use when**: Always, for serial delegation chains — it errs conservative under correlation. ⚠️ For *parallel defense layers* (verification stacks), the same multiplication applied to miss-rates errs **optimistic** under correlation — apply the common-cause correction from [Risk Propagation](/delegation-risk/risk-propagation/).
...
- [Delegation Risk Calculator](/design-patterns/tools/delegation-risk-calculator/) — Calculate Delegation Risk
- [Quick Start](/design-patterns/tools/quick-start/) — Apply these concepts to your system
- [Decision Guide](/design-patterns/tools/decision-guide/) — Choose implementation based on trust levels
