...
**Use when**: Failures must occur at multiple points simultaneously (redundant systems). ⚠️ This is exactly the case where correlation bites hardest — under common-cause correlation $\rho$, true combined failure approaches $\min(1-T_i)$ rather than $\prod(1-T_i)$. Budget with the mixture formula from [Risk Propagation](/delegation-risk/risk-propagation/).
