...
function printSummary(logs: QueryLog[]): void {
  ...
  const totalOutputTokens = successful.reduce((sum, l) => sum + l.outputTokens, 0);
  const avgDuration = successful.reduce((sum, l) => sum + l.durationMs, 0) / successful.length;
...
function printRecentLogs(logs: QueryLog[], count: number = 10): void {
  ...
  for (const log of recent) {
    ...
    console.log(`   Q: "${log.question.slice(0, 60)}${log.question.length > 60 ? "..." : ""}"`);
    if (log.success) {
      console.log(`   ${duration}s | ${log.toolCalls.length} tools | ${log.inputTokens + log.outputTokens} tokens | ${cost}`);
    } else {
