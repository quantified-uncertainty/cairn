...
- `metrics-extractor.mjs` (266 lines), `redundancy.mjs` (216 lines), `content-types.mjs` (133 lines), `file-utils.mjs` (102 lines), `output.mjs` (136 lines)

**WARNING: These files are currently UNTRACKED in git.** They exist on disk (created by a prior agent session / PR #5 work) but have never been committed. Any fresh clone or CI run will not have them. **Phase 1 Step 0 must commit these files before anything else.**
...
#### Step 1.0: Commit the untracked build scripts

The modularized build script and its lib/ modules exist locally in `apps/longterm-next/scripts/` but are **untracked in git**. They must be committed before any other work begins, or they'll be invisible to CI and other developers.

```bash
git add apps/longterm-next/scripts/
git commit -m "Add modularized build scripts for longterm-next"
```

Verify the files are present after commit:
- `scripts/build-data.mjs` (596 lines)
- `scripts/lib/entity-transform.mjs` (301 lines)
- `scripts/lib/computed-facts.mjs` (286 lines)
- Plus 7 other lib modules (content-types, file-utils, mdx-generator, metrics-extractor, output, redundancy, statistics, unconverted-links)
- `scripts/generate-llm-files.mjs`
- `scripts/flatten-content.mjs`
- `scripts/validate/`
...
**Note:** Currently `apps/longterm-next/src/data/database.json` does not exist — the runtime `getDatabase()` falls through to `apps/longterm/src/data/database.json`. After this step, longterm-next will have its own copy generated by its own build script. The fallback to longterm's copy should remain for backward compatibility during the transition.
...
### Phase 1 Completion Notes (2026-02-09)

All steps completed. Key outcomes:
- **Step 1.0**: Untracked build scripts committed as baseline (commit `f61b7826`)
...
- **Step 1.5**: Full Next.js build passes (exit code 0)
- **Additional**: `src/data/database.json` added to `.gitignore` (generated file)
- **Additional**: `yaml` package added as dependency (used by build scripts)
- **Critical bugs from red-teaming**: Both CRITICAL bugs (missing `scanFrontmatterEntities` and default case dropping extra fields) were fixed as part of this phase, ahead of schedule.
...
| **Untracked build scripts** — `longterm-next/scripts/` is not committed to git; CI and fresh clones won't have them | **HIGH** | Step 1.0 commits all untracked scripts before any other work. |
