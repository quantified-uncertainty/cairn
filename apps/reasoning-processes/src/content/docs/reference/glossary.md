...
| **Deception affordance** | A form's perceived likelihood ratio divided by its actual cost-to-fake one; operationally, its false-side belief swing relative to its true-side swing — near one, the form transmits persuasion, not information | [Untrustworthy Sources](/concepts/untrustworthy-sources/#measuring-it) |
...
| **Deception preconditions** | The conjunction (verification gap, reproduction gap, dependence, goal-divergence, never-resolves, stakes) all required for deception to be a live risk — break any one to defuse it; read as the defender's attack-surface-reduction checklist | [Untrustworthy Sources](/concepts/untrustworthy-sources/#move-1-reduce-the-attack-surface) |
| **Residual attack surface** | The few channels that survive design-time hardening — questions turning on an irreplaceable advantage, that never resolve, where goals diverge; where the runtime test must carry the load | [Untrustworthy Sources](/concepts/untrustworthy-sources/#the-residual-attack-surface) |
| **Irreplaceable advantage** | A source's output-relevant epistemic edge with no Blackwell-sufficient trusted substitute at feasible cost (replacement cost unbounded, though its value stays finite) — the core of the residual attack surface, where deception is genuinely dangerous | [Untrustworthy Sources](/concepts/untrustworthy-sources/#the-residual-attack-surface) |
| **Counterfactual-deceiver test** | The listener's runtime rule: use a message exactly to the degree a motivated liar in the source's position could not have produced it as cheaply if the claim were false | [Untrustworthy Sources](/concepts/untrustworthy-sources/#move-3-the-test) |
...
| $\tau$ | the listener's credence that a source is honest rather than strategic — underspecified as a scalar, refined into decision-indexed goal-divergence | [Untrustworthy Sources](/concepts/untrustworthy-sources/#move-1-reduce-the-attack-surface) |
