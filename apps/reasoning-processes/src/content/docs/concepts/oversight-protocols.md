...
Composition shrinks the judge's job to the unresolvable residue; the same move generalizes into a design discipline for the whole pipeline. Deception is a live risk only inside a [conjunction of conditions](/concepts/untrustworthy-sources/#move-1-reduce-the-attack-surface) — verification gap, reproduction gap, output-dependence, goal-divergence, never-resolves, stakes — so the program is to **route** as many sub-questions as possible out of that conjunction: make them self-checkable (break verification), trusted-reproducible (break reproduction), or eventually-resolvable (break never-resolves). This is the defender's design-time move: the [untrustworthy-sources chapter](/concepts/untrustworthy-sources/) frames routing as attack-surface reduction — the first move of a Stackelberg game, not a cleanup step. What remains is the [residual attack surface](/concepts/untrustworthy-sources/#the-residual-attack-surface) — questions that turn on an irreplaceable, output-critical advantage, never resolve, and where goals diverge — and on those you accept **trusted-frontier quality** (the Blackwell-best trusted experiment) unless hardening that specific question pays for itself. That fallback is the **defer-to-trusted** move of [AI control](https://arxiv.org/abs/2312.06942) (Greenblatt et al. 2024), generalized from runtime actions to question types.
