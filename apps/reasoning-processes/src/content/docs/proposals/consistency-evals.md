...
6. **Contextual consistency (prompt robustness).** Paraphrase invariance, framing neutrality, immunity to irrelevant added details, formatting robustness. A sharp special case is **label-swap neutrality**: on a decision the system has no information about, swapping the options should swap the output symmetrically — residual asymmetry measures [decision-relative bias](/concepts/process-catalogue/#reading-the-table) without any ground truth.
