 ...
 *
 * Usage:
 *   npm run crux -- fix cross-links              # Preview changes
 *   npm run crux -- fix cross-links --apply      # Apply changes
 *   npm run crux -- fix cross-links --verbose    # Show detailed matches
 *   npm run crux -- fix cross-links --file path  # Fix single file
...
function loadEntities() {
  ...
  if (!existsSync(registryPath) || !existsSync(dbPath)) {
    console.error('Error: Run npm run build:data first');
    process.exit(1);
  }
...
function validateCompiles(filePath, content) {
  ...
  try {
    ...
    execSync(`npx astro check ${tempPath} 2>&1`, {
      cwd: PROJECT_ROOT,
      stdio: 'pipe',
      timeout: 10000,
    });
...
async function main() {
  ...
  if (APPLY_MODE && totalChanges > 0) {
    ...
    console.log(`${colors.dim}Run 'npm run crux -- validate compile --quick' to verify${colors.reset}`);
  }
}
