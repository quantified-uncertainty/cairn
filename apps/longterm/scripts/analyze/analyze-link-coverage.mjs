 ...
 *
 * Usage:
 *   node scripts/analyze/analyze-link-coverage.mjs                    # Full report
 *   node scripts/analyze/analyze-link-coverage.mjs --orphans          # Show poorly-linked pages
 *   node scripts/analyze/analyze-link-coverage.mjs --top-linked       # Show most linked pages
 *   node scripts/analyze/analyze-link-coverage.mjs --json             # JSON output
 *   node scripts/analyze/analyze-link-coverage.mjs --page scheming    # Analyze specific page
 */
...
function loadBacklinks() {
  ...
  if (!existsSync(backlinksPath)) {
    console.warn('Warning: backlinks.json not found. Run npm run build:data first.');
    return {};
  }
...
function loadPathRegistry() {
  ...
  if (!existsSync(registryPath)) {
    console.warn('Warning: pathRegistry.json not found. Run npm run build:data first.');
    return {};
  }
...
function main() {
  ...
  console.log(`${colors.dim}Run npm run analyze:mentions to find unlinked entity mentions${colors.reset}`);
}
