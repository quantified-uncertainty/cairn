 ...
 *
 * Usage:
 *   node scripts/analyze/analyze-all.mjs           # Full health report
 *   node scripts/analyze/analyze-all.mjs --json    # JSON output
 *   node scripts/analyze/analyze-all.mjs --brief   # Summary only
 */
...
import { DATA_DIR_ABS as DATA_DIR } from '../lib/content-types.mjs';
...
async function main() {
  ...
  if (!BRIEF_MODE) {
    ...
    console.log(`${colors.dim}Run specific tools: npm run analyze:mentions | npm run analyze:links${colors.reset}`);
  }
}
