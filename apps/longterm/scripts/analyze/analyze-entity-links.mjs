 ...
 *
 * Usage:
 *   node scripts/analyze/analyze-entity-links.mjs sam-altman          # Analyze sam-altman
 *   node scripts/analyze/analyze-entity-links.mjs sam-altman --json   # JSON output
 *   node scripts/analyze/analyze-entity-links.mjs --help              # Show help
 *
...
function loadDatabase() {
  ...
  if (!existsSync(dbPath)) {
    console.warn('Warning: database.json not found. Run npm run build:data first.');
    return { entities: [] };
  }
...
function main() {
  ...
  if (!pathRegistry[entityId]) {
    ...
    console.log(`${colors.dim}Run: npm run crux -- analyze links --page ${entityId}${colors.reset}`);
    process.exit(1);
  }
