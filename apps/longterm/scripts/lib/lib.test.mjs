...
test('findMdxFiles returns array', () => {
  const result = findMdxFiles('src/content/docs/knowledge-base/models');
...
test('findFiles with extensions filter', () => {
  const result = findFiles('src/data', ['.yaml', '.yml']);
  assert(Array.isArray(result), 'Should return array');
...
test('getDirectories returns directories', () => {
  const result = getDirectories('src/content/docs/knowledge-base');
  assert(Array.isArray(result), 'Should return array');
...
test('CONTENT_DIR is set correctly', () => {
  assertEqual(CONTENT_DIR, 'src/content/docs');
});
