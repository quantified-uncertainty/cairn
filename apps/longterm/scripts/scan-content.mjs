 ...
 *
 * Usage:
 *   node scripts/scan-content.mjs [options]
 *
 * Options:
...
function processEntityRelations() {
  if (!existsSync(ENTITIES_PATH)) {
    console.log(`${colors.yellow}⚠️  entities.yaml not found${colors.reset}`);
    return 0;
  }

  const entitiesYaml = readFileSync(ENTITIES_PATH, 'utf-8');
  const entities = parseYaml(entitiesYaml) || [];
