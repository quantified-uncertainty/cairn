 ...
 *
 * Usage:
 *   node scripts/fix-broken-links.mjs                    # Report broken links
 *   node scripts/fix-broken-links.mjs --dry-run          # Show what would be fixed
 *   node scripts/fix-broken-links.mjs --fix              # Auto-fix (prefer EntityLink)
 *   node scripts/fix-broken-links.mjs --fix --remove     # Remove broken links entirely
 *   node scripts/fix-broken-links.mjs --interactive      # Ask about each file
 *
 * Strategies:
...
const PATH_REGISTRY_FILE = join(process.cwd(), 'src/data/pathRegistry.json');
const DATABASE_FILE = join(process.cwd(), 'src/data/database.json');
let pathRegistry = {};
