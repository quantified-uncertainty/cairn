...
function loadGaps() {
  ...
  const { insights } = parseYaml(readFileSync(INSIGHTS_PATH, 'utf-8'));
 ...
 */
async function extractInsights(page, content, model) {
...
async function main() {
  ...
  for (const page of gaps) {
    ...
    try {
      ...
      const rawInsights = await extractInsights(page, content, options.model);
  ...
  // Apply if requested
  if (options.apply) {
    const existing = parseYaml(readFileSync(INSIGHTS_PATH, 'utf-8'));
    const cleaned = allInsights.map(ins => {
      const { _evidence, _fromPage, ...clean } = ins;
      return clean;
    });

    existing.insights = [...(existing.insights || []), ...cleaned];

    writeFileSync(INSIGHTS_PATH, stringifyYaml(existing, { lineWidth: 120 }));
    console.log(`\x1b[32m✓ Added ${cleaned.length} insights to insights.yaml\x1b[0m`);
  } else {
    console.log('\n\x1b[33mReview proposals, then run with --apply to add to insights.yaml\x1b[0m');
  }
}
