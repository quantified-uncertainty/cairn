...
import graphYaml from './graphs/safety-research-generalizability.yaml?raw';
...
if (validationErrors.length > 0) {
  console.error('Safety generalizability graph validation errors:');
...
export const safetyGeneralizabilityNodes: Node<CauseEffectNodeData>[] = rawData.nodes.map(node => ({
  id: node.id,
  type: 'causeEffect',
...
export const safetyGeneralizabilityEdges: Edge<CauseEffectEdgeData>[] = rawData.edges.map(edge => ({
  id: edge.id,
  source: edge.source,
  target: edge.target,
  data: {
...
// Export raw data for table view and other uses
export const rawGraphData = rawData;
