...
ratings:
  ...
  completeness: 6
importance: 71.5
llmSummary: This model estimates alignment robustness degrades from 60-80% at GPT-4 level to 30-50% at 100x capability, with a critical 'alignment valley' at 10-30x where systems are dangerous but not helpful for alignment research. The framework decomposes robustness into training (R_train), deployment (R_deploy), and intent preservation (R_intent) components, estimating the most dangerous period arrives in 2-5 years.
