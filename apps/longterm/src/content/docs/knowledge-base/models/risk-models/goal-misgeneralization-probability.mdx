...
ratings:
  ...
  completeness: 7.3
importance: 78.5
llmSummary: "Quantitative framework estimating goal misgeneralization probability from 3.6% (superficial shifts) to 27.7% (extreme shifts), with modifiers for specification quality (0.5-2.0x), capability level (0.5-3.0x), and alignment methods (0.4-1.5x). Meta-analysis of 60+ cases shows 87% capability transfer, 76% goal failure rate, providing actionable deployment guidance and intervention effectiveness estimates (interpretability: 40-70% risk reduction, AI control: 60-90%)."
