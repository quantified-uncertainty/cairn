...
importance: 60.5
llmSummary: World models + planning (AlphaGo, MuZero, DreamerV3) achieve 10-500x better sample efficiency than model-free RL and superhuman game performance, but estimated only 5-15% probability of TAI dominance due to LLM success on general tasks. Key advantages include explicit goals and inspectable beliefs; key risks include goal misgeneralization and mesa-optimization.
ratings:
  novelty: 4.5
  rigor: 6.5
  actionability: 5
...
| **Key Advocate** | Yann LeCun (Meta) | Argues LLMs are "dead end"; JEPA is path to AGI |
