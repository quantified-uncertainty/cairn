...
importance: 44.5
llmSummary: "Neuromorphic hardware (Intel Loihi 2, IBM TrueNorth, etc.) achieves 100-1000x energy efficiency over GPUs for sparse inference through brain-inspired spiking neural networks, but remains 15%+ behind transformers on accuracy benchmarks and lacks proven scaling laws. Estimated 1-3% probability of being dominant at TAI due to fundamental capability gaps, though could become relevant if energy constraints force paradigm shift or algorithmic breakthroughs enable SNN scaling."
ratings:
  novelty: 4.2
  rigor: 6.1
  actionability: 3.8
