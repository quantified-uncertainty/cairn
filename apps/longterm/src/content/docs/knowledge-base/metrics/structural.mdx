...
importance: 55
quality: 58
llmSummary: "Comprehensive compilation of 10 structural metrics tracking society's capacity to govern AI, including information environment quality (Freedom House scores declining), institutional decision-making (WGI proxies), elite-public divergence (72% want slower development vs expert optimism), policy lag times (1-3 years EU/US), coordination failures (non-binding agreements), capability concentration (HHI ~2,050-2,500 for LLM market), and incident reporting (2,000+ in AIID but near-miss reporting minimal). Most metrics are measured annually with medium data quality, though several key dimensions (coordination success rates, AI-specific resilience) remain conceptual with qualitative assessment only."
ratings:
  novelty: 4.5
  rigor: 6
  actionability: 4
