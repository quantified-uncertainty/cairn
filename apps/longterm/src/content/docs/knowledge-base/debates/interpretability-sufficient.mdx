...
**Key insight from Chris Olah (July 2024)**: Despite major progress on superposition and scalability, we are "only extracting a small fraction of the features"—a "neural network dark matter" problem. (See [Interpretability Coverage](/ai-transition-model/parameters/interpretability-coverage/) for the full quote and analysis.)
