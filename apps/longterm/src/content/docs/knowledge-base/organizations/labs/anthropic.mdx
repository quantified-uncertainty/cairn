...
sidebar:
  order: 1
quality: 46
llmSummary: Anthropic is an AI safety company with ~1,000 employees and $7B+ funding that developed Constitutional AI (82% reduction in harmful outputs) and achieved major interpretability breakthrough (16M features extracted from Claude). Critical uncertainties include whether commercial pressure ($7.3B raised, not yet profitable) will undermine safety priorities and whether their self-governed Responsible Scaling Policy can prevent dangerous deployments as they approach ASL-3 threshold capabilities by 2025.
