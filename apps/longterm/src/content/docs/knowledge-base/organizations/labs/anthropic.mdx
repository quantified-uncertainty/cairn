...
Anthropic and <EntityLink id="openai">OpenAI</EntityLink> are the two leading frontier AI companies. Updated January 2026 data:

| Metric | Anthropic | OpenAI | Assessment |
|--------|-----------|--------|------------|
| **Valuation** | \$350B | \$500B (targeting \$750-830B) | OpenAI larger |
| **Revenue (ARR)** | \$9B (end 2025) | \$20B (Jan 2026) | OpenAI 2.2x higher |
| **Revenue multiple** | ≈39x | ≈25x (current) | Anthropic trades richer |
| **Enterprise retention** | 88% | Unknown | Anthropic leads (vs 76% industry avg) |
| **Coding benchmarks** | 80.9% SWE-bench | 74.9% | Claude leads coding |
| **Path to breakeven** | 2028 | Unknown | Anthropic more transparent |

**Key risks**: ≈25% of Anthropic's revenue comes from just two customers (Cursor and GitHub Copilot). Gross margin forecast was cut from 50% to 40% due to rising inference costs.

**Talent concentration** remains a strength: The founding team includes 7 ex-OpenAI researchers (GPT-3 lead author Tom Brown, scaling laws pioneer Jared Kaplan, interpretability founder <EntityLink id="chris-olah">Chris Olah</EntityLink>). Recent acquisitions include <EntityLink id="jan-leike">Jan Leike</EntityLink> (former OpenAI Superalignment co-lead) and John Schulman (OpenAI co-founder, PPO inventor). The interpretability team of 40-60 researchers is the largest globally.

For detailed bull and bear case arguments, see <EntityLink id="anthropic-valuation">Anthropic Valuation Analysis</EntityLink>. For IPO timeline and extended growth scenarios, see <EntityLink id="anthropic-ipo">Anthropic IPO</EntityLink>. For implications on EA-aligned philanthropic capital, see <EntityLink id="anthropic-investors">Anthropic (Funder)</EntityLink>.
