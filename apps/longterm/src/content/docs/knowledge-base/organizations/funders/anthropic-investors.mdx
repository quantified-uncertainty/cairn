...
importance: 78
llmSummary: "Comprehensive model of EA-aligned capital at Anthropic. At $350B valuation: $25-70B risk-adjusted (only 2/7 founders—Dario and Daniela Amodei—have strong EA connections). Extended growth scenarios model 1.5-5x valuations ($500B-$1.75T) yielding $35B-350B+ EA capital. Valuation context: Anthropic trades at ~39x revenue vs OpenAI's ~25x (at current $500B); both at similar multiples if OpenAI closes $830B round. Key strengths include 88% enterprise retention (vs 76% industry average) and largest interpretability team globally (40-60 researchers). Key risks include 25% customer concentration (Cursor + GitHub Copilot) and margin pressure (forecast cut from 50% to 40%). Matching program reduced from 3:1@50% to 1:1@25% for new employees. Timeline: employee capital 2027-2030; founder capital 2030-2040."
ratings:
  novelty: 5
  rigor: 5
  actionability: 6
