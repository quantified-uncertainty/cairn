...
sidebar:
  order: 15
quality: 91
llmSummary: METR conducts pre-deployment dangerous capability evaluations for frontier AI models across autonomy, cybersecurity, CBRN, and manipulation domains, with quantitative findings showing task completion time horizons doubling every 7 months (accelerating to 4 months in 2024-2025). GPT-5 achieves 2h17m median horizon vs Claude 3.7 at ~50 minutes. Funded by 17M USD via The Audacious Project, they have evaluated models for OpenAI, Anthropic, and DeepMind, analyzed 12 companies' safety policies, and partnered with UK/US AI Safety Institutes. Key outputs include a 77-task autonomy evaluation suite and the MALT dataset (10,919 transcripts, 0.96 AUROC for reward hacking detection).
