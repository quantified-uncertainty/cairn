...
sidebar:
  order: 15
quality: 63
llmSummary: METR conducts pre-deployment dangerous capability evaluations for frontier AI models across autonomy, cybersecurity, CBRN, and manipulation domains, with quantitative findings showing task completion time horizons doubling every 4-7 months (GPT-5 at 2h17m, Claude 3.7 at ~50 minutes). Their assessments directly influence deployment decisions at OpenAI, Anthropic, and DeepMind, though fundamental challenges remain around capability hiding, evaluation gaming, and whether current methodologies can scale to AGI-level systems.
