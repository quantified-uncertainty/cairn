...
llmSummary: RLHF and Constitutional AI are the dominant techniques for aligning current language models, with InstructGPT (1.3B) preferred over GPT-3 (175B) 85±3% of the time, and GPT-4 showing 82% reduction in disallowed content. However, fundamental scalability limitations emerge at superhuman capability levels where human evaluation becomes unreliable. Key challenges include reward hacking, sycophancy (which worsens with model size), and inter-annotator disagreement (~25%), with effectiveness estimates of 20-40% risk reduction for current systems but uncertain scaling beyond human-level capabilities.
