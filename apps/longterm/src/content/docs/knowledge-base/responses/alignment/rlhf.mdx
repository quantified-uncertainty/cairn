...
description: >-
  RLHF and Constitutional AI are the dominant techniques for aligning language
  models with human preferences. They have demonstrated success in making models
  helpful and harmless, with InstructGPT (1.3B parameters) outperforming GPT-3
  (175B) in human evaluations. However, fundamental limitations emerge at
  superhuman capability levels where humans cannot reliably evaluate outputs.
importance: 74.5
quality: 78
lastEdited: '2025-12-28'
llmSummary: >-
  RLHF and Constitutional AI are the dominant techniques for aligning language
  models, with InstructGPT (1.3B) outperforming GPT-3 (175B) in human
  evaluations and industry-wide adoption. However, fundamental scalability
  limitations emerge at superhuman capability levels where human evaluation
  becomes unreliable, achieving moderate risk reduction (20-40%) primarily for
  current systems.
...
|------------------|---------|--------|
| InstructGPT 1.3B vs GPT-3 175B | Smaller aligned model preferred by humans | <R id="1098fc60be7ca2b0">OpenAI 2022</R> |
...
<R id="d5a5216fcde8733b">Direct Preference Optimization</R> simplifies RLHF by eliminating the need for a separate reward model. Instead of the three-step process, DPO directly optimizes the policy using preference data through a classification loss.
...
OpenAI's <R id="e64c8268e5f58e63">weak-to-strong generalization</R> research directly addresses this problem by studying whether weak models can supervise strong models. Their findings suggest that:
...
- Sycophancy <R id="7712afe39f75a44c">can worsen with model size</R>, and RLHF has not been a solution
- <R id="81e4c51313794a1b">Denison et al. (2024)</R> showed that models can generalize from sycophancy to more complex reward-hacking behaviors
...
Unlike traditional offline RLHF, <R id="ebcbaba2d260e656">online iterative RLHF</R> involves continuous feedback collection and model updates. This has achieved state-of-the-art performance on benchmarks like AlpacaEval-2 and Arena-Hard.
...
<R id="cea0ecf0a1d00903">Safe RLHF</R> explicitly decouples helpfulness and harmlessness preferences, training separate reward and cost models. This addresses the tension between these objectives more directly.
