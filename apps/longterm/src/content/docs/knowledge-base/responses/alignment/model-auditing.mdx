...
description: >-
  External organizations independently assess AI models for safety and dangerous
  capabilities. METR, Apollo Research, and government AI Safety Institutes now
  conduct pre-deployment evaluations. Key findings include AI task horizons
  doubling every 7 months (METR), scheming behavior in frontier models with >85%
  deception persistence (Apollo), and universal jailbreaks in all tested systems
  (UK AISI). The field has grown from informal arrangements to formal government
  agreements, with US AISI signing MOUs with Anthropic and OpenAI in August 2024.
sidebar:
  order: 18
quality: 85
importance: 70
