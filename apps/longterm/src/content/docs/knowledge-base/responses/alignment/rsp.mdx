...
llmSummary: Comprehensive analysis of Responsible Scaling Policies (RSPs) - voluntary commitments by 20 AI companies to pause scaling when capability thresholds are crossed. Documents three major frameworks (Anthropic ASL-3, OpenAI High/Critical, DeepMind CCL) with SaferAI grades of 1.9-2.2/5. METR has evaluated 5+ models pre-deployment. Effectiveness depends critically on voluntary compliance (0 major violations documented), evaluation science keeping pace with ~7-month capability doubling, and competitive pressures among 3+ frontier labs.
ratings:
  novelty: 5
  rigor: 7.5
  actionability: 7.5
