...
sidebar:
  order: 2
llmSummary: AI-assisted alignment uses current AI systems to help solve alignment problems through automated red-teaming (reducing jailbreak success from 86% to 4.4%), interpretability (extracting 10M features from Claude 3), and weak-to-strong generalization (GPT-2 supervision recovering GPT-3.5-level performance on GPT-4). Evidence shows significant capability gains for specific tasks, but scalability to superhuman systems remains uncertain and the bootstrapping problem (using potentially misaligned AI to align stronger AI) poses fundamental risks.
