...
Components pull from YAML databases:

```mdx
import {EntityLink, DataEstimateBox, DataInfoBox} from '@components/wiki';

{/* Link to entity with automatic title lookup */}
<EntityLink id="deceptive-alignment">deceptive alignment</EntityLink>

{/* Pull estimate data from estimates.yaml */}
<DataEstimateBox dataId="p-doom" />

{/* Display expert info from experts.yaml */}
<DataInfoBox expertId="paul-christiano" />
```

**For database details, see [Content Database](/internal/content-database/).**
...
- [Content Database](/internal/content-database/) — SQLite caching and AI summaries
