...
```bash
npm run validate:mdx-compile    # Syntax errors
npm run validate:dollars        # Unescaped $
npm run validate:comparisons    # Unescaped < >
npm run validate:entity-links   # Broken links
```
...
For CAIRN: Start with **Perplexity Sonar via OpenRouter** for deep research, and **Brave or Tavily** for quick grounding. Both have MCP servers for easy Claude Code integration.
</Aside>
...
Scry is particularly interesting for CAIRN because it includes curated content from LessWrong, EA Forum, AI safety research, and alignment papers—exactly the sources we cite most.
</Aside>
